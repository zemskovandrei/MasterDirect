-- Единая таблица specialist: мастера, бригады, мебельщики.
-- Только таблицы: specialist, order, site_reviews.

alter table public.specialist
  add column if not exists slug text,
  add column if not exists account_type text;

-- Расширить допустимые значения account_type (worker | brigade | furniture).
comment on column public.specialist.account_type is 'worker | brigade | furniture';
